          ...
          <div>
            ...
            <ul className="mt-8 grid gap-3 text-sm text-slate-700 md:grid-cols-2">
              {page.hero.summaryBullets.map((bullet) => (
                <li key={bullet} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">{bullet}</li>
              ))}
            </ul>
            <p className="mt-6 text-sm text-muted">{page.hero.trustCue}</p>
          </div>
          <figure className="overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-100 shadow-sm">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-10 lg:grid-cols-[260px_1fr] lg:px-8">
        <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-6">
          ...
          <nav className="mt-4 grid gap-2 text-sm text-muted">
            {page.toc.map((item) => (
              <a key={item.id} href={`#${item.id}`} className="rounded-xl px-3 py-2 hover:bg-slate-50 hover:text-ink">{item.label}</a>
            ))}
          </nav>
        </aside>
        ...
        <article className="grid gap-8">
          {page.sections.map((section) => {
            ...
            return (
              <section id={section.id} key={section.id} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                <p className="text-sm font-medium text-blue-700">Intent: {section.searchIntent}</p>
                <h2 className="mt-3 text-3xl font-semibold tracking-tight text-ink">{section.heading}</h2>
                <p className="mt-4 rounded-2xl bg-blue-50 p-4 text-blue-900">{section.summaryClaim}</p>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {page.qualityGates.map((gate) => (
                <div key={gate.id} className={`rounded-2xl border p-4 ${statusClass(gate.status)}`}>
                  <div className="flex items-center justify-between gap-3">
                    <strong>{gate.label}</strong>
                    <span className="rounded-full bg-white/70 px-2 py-1 text-xs uppercase">{gate.status}</span>
                  </div>
                  <p className="mt-2 text-sm">{gate.evidence}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {page.internalLinks.map((link) => (
                <a key={link.href} href={link.href} className="rounded-2xl border border-slate-200 p-4 hover:border-blue-300 hover:bg-blue-50">
